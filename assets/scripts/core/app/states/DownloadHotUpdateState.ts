...
export class DownloadHotUpdateState extends GameFlowStateBase {
  public readonly id = GameState.DownloadHotUpdate;
  ...
  public override async enter(): Promise<void> {
    ...
    const result = await hotUpdate.applyUpdate();

    if (result.restartRequired) {
      await this.transition(GameState.RestartRequired);
      return;
    }
